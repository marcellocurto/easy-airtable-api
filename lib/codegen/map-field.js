...
export function mapFieldTypes({ field, enumMode, unknownFieldBehavior, }) {
    ...
    const linkedTableId = getRecordValue(field.options, 'linkedTableId');
    switch (field.type) {
        case 'singleLineText':
        case 'multilineText':
        case 'richText':
        ...
        case 'formula': {
            const result = mapResultDescriptor(getRecordValue(field.options, 'result'), field, enumMode, unknownFieldBehavior);
            return {
                read: result,
                readonly,
                computed,
...
export function mapFieldToNormalizedField({ field, enumMode, unknownFieldBehavior, tsKey, tsTypeName, }) {
