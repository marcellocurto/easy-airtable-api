import { makeUniqueConstKeys, makeUniquePascalNames, toPascalCase, toSafeConstKey, } from './identifiers.js';
...
export function normalizeSchema({ schema, tableNameOrId, enumMode, unknownFieldBehavior, }) {
    ...
    const tables = selectedTables.map((table) => {
        ...
        if ((tableConstKeyCounts.get(toSafeConstKey(table.name)) ?? 0) > 1) {
            warnings.push(`Table const key collision detected for ${table.name}. Generated tables key ${tsConstKey} includes a stable suffix.`);
        }
        ...
        const fields = table.fields.map((field) => {
            ...
            if ((exactFieldNameCounts.get(field.name) ?? 0) > 1) {
                warnings.push(`Duplicate field name detected for ${table.name}.${field.name}. Generated interfaces merge the colliding fields into a union.`);
            }
            ...
            if ((fieldConstKeyCounts.get(toSafeConstKey(field.name)) ?? 0) > 1) {
                warnings.push(`Field const key collision detected for ${table.name}.${field.name}. Generated fields key ${normalizedField.tsConstKey} includes a stable suffix.`);
            }
