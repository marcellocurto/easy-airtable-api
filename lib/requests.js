...
export async function generateTypeScriptDefinitions({ apiKey, baseId, tableNameOrId, }) {
    const schema = await getBaseSchema({ apiKey, baseId });
    const table = schema.tables.find((t) => t.id === tableNameOrId || t.name === tableNameOrId);
    if (!table) {
        throw new Error(`Table with name or ID "${tableNameOrId}" not found.`);
    }
    let typeDefinitions = `type ${table.name}Fields = {\n`;
    table.fields.forEach((field) => {
        typeDefinitions += `  ${field.name}?: ${mapAirtableTypeToTypeScript(field.type)};\n`;
    });
    typeDefinitions += '};\n';
    return typeDefinitions;
}
function mapAirtableTypeToTypeScript(airtableType) {
    switch (airtableType) {
        case 'aiText':
        case 'singleLineText':
        case 'email':
