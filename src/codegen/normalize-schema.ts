...
export function normalizeSchema({
  ...
  const sanitizedTableNameCounts = new Map<string, number>();

  selectedTables.forEach((table) => {
    const baseName = toPascalCase(table.name);
    sanitizedTableNameCounts.set(
      baseName,
      (sanitizedTableNameCounts.get(baseName) ?? 0) + 1
    );
  });
  ...
  const tables: NormalizedTable[] = selectedTables.map((table) => {
    ...
    const sanitizedFieldNameCounts = new Map<string, number>();
    table.fields.forEach((field) => {
      const baseName = toPascalCase(field.name);
      sanitizedFieldNameCounts.set(
        baseName,
        (sanitizedFieldNameCounts.get(baseName) ?? 0) + 1
      );
    });
